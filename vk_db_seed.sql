...
SELECT 
	...
	(SELECT CONCAT(u.firstname, " ", u.lastname) FROM users u WHERE u.id = target_user_id) AS friend,
    f.status AS `status`
FROM users u
JOIN friend_requests f ON f.initiator_user_id = u.id
WHERE u.id = 1 AND f.status = 'approved';
...
SELECT
	...
	COUNT(m.id) OVER w AS total_likes
FROM media m
...
SELECT 
    ...
    COUNT(c.`name`) OVER w AS total_groups
FROM users u
JOIN users_communities us ON u.id = us.user_id
JOIN communities c ON us.community_id = c.id
WINDOW w AS (PARTITION BY u.id);

-- 1. Создайте представление, в которое попадет информация о пользователях (имя, фамилия, город и пол), которые не старше 20 лет.
    
CREATE OR REPLACE VIEW profile_user
AS SELECT
	CONCAT(u.firstname, " ", u.lastname) AS fullname,
    TIMESTAMPDIFF(YEAR, p.birthday, CURDATE()) AS birthday,
    p.hometown AS town
FROM users u
...
SELECT 
	CONCAT(u.firstname, " ", u.lastname) AS fullname,
    COUNT(m.from_user_id) AS total_messages,
   DENSE_RANK() OVER(ORDER BY COUNT(m.from_user_id) DESC) AS `rating`
FROM users u
