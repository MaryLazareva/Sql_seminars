...
VALUES 
    ...
    SELECT 
		a.an_name AS `name`, 
        a.an_price AS price, 
        DATE(o.ord_datetime) AS `date`
    FROM analysis a
    JOIN orders o ON a.an_id = o.ord_an
	WHERE DATE(o.ord_datetime) BETWEEN '2020-02-05' AND DATE_ADD('2020-02-05', INTERVAL 1 WEEK)
    ORDER BY `date`;
...
WHERE TIMESTAMPDIFF(YEAR, p.birthday, CURDATE()) < 12; -- функция TIMESTAMPDIFF используется для вычисления  разницы в годах
                                                      -- между датой рождения пользователя (p.birthday) и текущей датой (CURDATE())
 
 -- Определить кто больше поставил лайков (всего): мужчины или женщины.

SELECT
    SUM(CASE WHEN p.gender = 'f' THEN 1 ELSE 0 END) AS female_likes, 
    SUM(CASE WHEN p.gender = 'm' THEN 1 ELSE 0 END) AS male_likes
FROM `profiles` p
...
SELECT 
	CONCAT(u.firstname, " ", u.lastname) AS fullname,
    m.to_user_id,
    (SELECT CONCAT(u.firstname, " ", u.lastname) FROM users u WHERE u.id = m.from_user_id) as person_who_send_messages,
    COUNT(*) AS send_messages
FROM users u
JOIN messages m ON u.id = m.to_user_id
WHERE u.id = 1
GROUP BY m.from_user_id
ORDER BY send_messages DESC
LIMIT 1;
